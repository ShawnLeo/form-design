<template>
	<div class="input-config">
		...
		<Form ref="inputFrom" :model="config"  label-position="top">
			...
			<FormItem label="字段最大长度" prop="maxLength">
				<InputNumber v-model="config.maxLength" placeholder="字段最大长度（默认80）"/>
			</FormItem>
...
<script>
	export default {
		name: 'mmt-textarea-config',
		props: {
			config: Object
		},
		computed: {
			bindKey () {
				return this.config.bindKey ? this.config.placeholder : this.config.bindKey;
			}
		}
	};
</script>
