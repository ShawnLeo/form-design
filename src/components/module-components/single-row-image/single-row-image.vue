<template>
	<div class="single-image-list" :class="[{'is-padding': config.padding.value === 'yes'}, {'is-round': config.round.value === 'yes'}]">
		<div class="single-row" v-for="(item, index) in cdata" :key="index">
			<div class="single-row-container">
				<a href="javascript:;">
					<img src="data:image/png;base64,"
						class="single-row-image">
				</a>
			</div>
		</div>
	</div>
</template>

<script type="text/ecmascript-6">
	import Mixin from '../mixin';
	export default {
		name: 'single-row-image',
		mixins: [Mixin],
		computed: {},
		mounted() {}
	};
</script>

<style scoped>
	.single-image-list{
		width: 100%;
		background-color: #ffffff;
	}

	.single-row-container a{
		display: block;
		font-size: 0;
	}

	.is-padding .single-row {
		padding: .2rem;
	}

	.is-round .single-row-container {
		border-radius: .2rem;
		overflow: hidden;
	}

	.single-row-image{
		width: 100%;
		height: auto;
		background-size: cover;
		background-repeat: no-repeat;
	}

</style>